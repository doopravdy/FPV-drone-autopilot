...
def ai_thread():
    ...
    if mask_path and os.path.exists(mask_path):
        ...
        if mask is not None:
            # 检查掩膜尺寸是否符合预期 (可选警告)
            if mask.shape[0] != cfg['img_height'] or mask.shape[1] != cfg['img_width']:
                print(f"⚠ 警告: 掩膜尺寸 {mask.shape} 与配置 {cfg['img_height']}x{cfg['img_width']} 不一致")
    ...
    # 2. 初始化模型 (注意 input_shape 顺序为 Channel, Height, Width)
    model = CNNLSTM(
        input_shape=(3, cfg['img_height'], cfg['img_width']),  # 修改点: 使用 H, W
        lstm_hidden=cfg['lstm_hidden'],
        lstm_layers=cfg['lstm_layers'],
        output_dim=4
    ).to(device)
    ...
    # 4. 帧缓冲区初始化 (尺寸修改为 80x128x3)
    frame_buffer = deque(maxlen=cfg['seq_len'])
    dummy_frame = np.zeros((cfg['img_height'], cfg['img_width'], 3), dtype=np.float32)
    for _ in range(cfg['seq_len']):
        frame_buffer.append(dummy_frame)
    ...
    with mss.mss() as sct:
        ...
        while running:
            ...
            if mask is not None:
                # 直接缩放到掩膜的尺寸 (128x80)
                mask_h, mask_w = mask.shape  # 80, 128

                # resize 参数顺序: (width, height)
                frame = cv2.resize(frame, (mask_w, mask_h))

                # 应用掩膜
                mask_3ch = np.stack([mask, mask, mask], axis=-1)
                frame = frame * mask_3ch
            else:
                # 没有掩膜，直接缩放到目标尺寸
                frame = cv2.resize(frame, (cfg['img_width'], cfg['img_height']))
            ...
            # D. 构建 Tensor (Batch, Seq, Channel, H, W)
            sequence = np.array(list(frame_buffer))
            tensor = torch.from_numpy(sequence).permute(0, 3, 1, 2)
            tensor = tensor.unsqueeze(0).to(device)
